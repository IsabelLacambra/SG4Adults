...
public class equipo04 : MonoBehaviour {
	...
	public GameObject puntosEq4;
	Puntos04 miPuntuacion4; 
	...
	void Start () {
		miPuntuacion4 = (Puntos04)puntosEq4.GetComponent (typeof(Puntos04));
	...
	void funcionlarga(){
		...
		if(string.Equals(ultimaClickada, "2")){
			...
			if(celdaInVecinos(vecinosMyPos,posicion)){
				if(miPuntuacion4.PuntosEq04 >= 10) {
					Debug.Log("Fin");
					Application.LoadLevel ("Scene07");
				}
			}
